...
$attempts = array(1=>1, 2=>2, 3=>2 , 4=>2 ,5=>2, 6=>2);  //change the attempts of 1 and 2. 

//number of complaints for each user of each day is checked.
//if the user has made any complaints.

/*if(isset($_SESSION["u1@gmail.com"]))
	print_r($_SESSION["u1@gmail.com"]); echo " array1"."<br>"; 
if(isset($_SESSION["u1@gmail.comcount"]))
	print_r($_SESSION["u1@gmail.comcount"]);echo " count1"."<br>";

if(isset($_SESSION["u2@gmail.com"]))
	print_r($_SESSION["u2gmail.com"]);echo " array2"."<br>";
if(isset($_SESSION["u2@gmail.comcount"]))
	print_r($_SESSION["u2@gmail.comcount"]);echo " count2"."<br>";

if(isset($_SESSION["u3@gmail.com"]))
	print_r($_SESSION["u3@gmail.com"]);echo " array3"."<br>";
if(isset($_SESSION["u3@gmail.comcount"]))
	print_r($_SESSION["u3@gmail.comcount"]);echo " count3"."<br>";*/
	...
	if($_SESSION[$userid."count"]>$attempts[$_SESSION["rat"]])
	{
		...
		if(isset($_SESSION[$temp]))
		{
			...
			for($i=0;$i<sizeof($_SESSION[$userid]);$i++)
			{
				...
				$del = "delete from crime where cid='$mil'";
				$resdel = $conn->query($del) or die("Error connecting to the crime table of the database.");;
				if($score >= 4)  //  score decreaed to 2 increase it to 4
				{
					$pos=$pos+1;
					for($j=0;$j<sizeof($usernames);$j++)
					{
						$tempo=$usernames[$j];
						$u_sql="update user set Rating = Rating + 2 where Email = '$tempo'";
				...
				for($k=0;$k<sizeof($usernames);$k++)
				{
					...
					{
						$cars=$_SESSION[$usernames[$k]];
						if (($key = array_search($mil, $cars)) !== false) {
    							unset($cars[$key]);
							}
					}
				}
...
else
{
	...
	$sqly ="insert into crime(cid,uid) values('$crimeid','$userid')";
		$resulty = $conn->query($sqly) or die("Error connecting to the crime table of the database.");
	echo "<script>window.open('garb.html','_self')</script>";
